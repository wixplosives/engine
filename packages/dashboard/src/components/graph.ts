...
import { generateColorPicker, size, updateLinks, updateNodes } from './dependency-graph/graph-drawing-helpers'
import type { Stage } from "./dependency-graph/dependency-graph";
...
export function updateGraph(data: SerializedGraphData, selected: string, setSelected: (name: string) => void, stage: Stage) {
    if (data.nodes.length > 0) {
        ...
        const activeLinks = () => {
            ...
            return nodes.some(n => n.isSelected)
                ? links.filter(l => l.distance > 0)
                : links;
        }
        let color = generateColorPicker(nodes)

        const linksGroup = stage.links
        const nodesGroup = stage.nodes
        ...
        const _setSelection = (_: any, selected: NodeDatum) => {
            ...
            updateLinks(linksGroup, activeLinks(), color)
