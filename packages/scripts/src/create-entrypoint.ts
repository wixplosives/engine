...
async (configName: string, envName: string, featureName: string, fetchedConfigs: Record<string, string>) => {
    const config = await (await fetch('configs/' + configName + '?env=' + envName + '&feature=' + featureName)).json() as string;
